...
class Agent:
    ...
    def update_train(self,a, b):
        if self._rl_memory.size() > self.batch_size:
            ...
            next_continuous_param = self.actor.target_model.predict(next_states)
            target_qs_list = self.model_target.predict([next_states, next_continuous_param])  # output所有的Q值
            # 连续网络如何更新参数
    ...
    def concat_dis_conti(self, discrete_action, continuous_action):
        # tuple (batchsize, 1, 2)
        concat_actions = []
        for i in range(len(discrete_action)):
            temp_a = np.array(discrete_action[i][0]).flatten()
            temp_b = np.array(continuous_action[i][0]).flatten()
            concat_actions.append(temp_a + temp_b)
        return concat_actions
    ...
    def update_strategy(self):  #连续动作
        if self._rl_memory.size() > self.batch_size: #todo
            ...
            actions = np.array([e[1] for e in batch])  # 组合action 【discrete/continuous】  type (batch, 1, 4)
            '''
            discrete_actions = []
            continuous_actions = []
            ...
            for k in range(len(batch)):
                if y_t[k] is None or target[k] is None or rewards[k] is None:
                    ...
                    #y_t[k] = rewards[k] + self.gamma * target[k]
            loss += self.critic.model.train_on_batch([states, actions], y_t)  # y_t == target q
            self.loss.append(loss)
            a_for_grad = self.actor.model.predict(states) # tuple(batch_size, 1, 4)
            grads = self.critic.gradients(states, a_for_grad)  # loss 上的梯度   gradient的action是4维 type (batch_size,1,4)
            self.grad.append(grads)
            # update actor policy using sampled gradient
    ...
    '''
    def actor_output(self, state, trader_type, type, count, market_number, a, b):
        # 产生所有的离散和连续动作组合
        a = self.act_best_response(np.reshape(state, (1, 1, self.state_dim)))  # 输出离散动作和连续动作组合
        a_discrete = []
        a_continuous = []
        #分离discrete 和 continuous
        for i in range(self.action_dim[0]):
            a_discrete.append(a[0][0][i])
        for j in range(self.action_dim[1]):
            a_continuous.append(a[0][0][j + self.action_dim[0]])

        # 可以对param加参数
        a_continuous_original = copy.copy(a_continuous)
        ...
        k = left_episode / self.episodes
        # k = 1
        if trader_type == 'S':
            for i in range(self.action_dim[1]):  # 对每个维度的action判断
                temp_a = a_continuous_original[i]
        ...
        if trader_type == 'B':  # 对每个维度判断
            for j in range(self.action_dim[1]):
                temp_b = a_continuous_original[j]
                if temp_b > type:  # 原始出价不合理
                    if random.random() < 0.3:  # 探索不合理的action
                        noise[j] = 0
                        a_new[j] = temp_b + 0
                    else:  # 探索合理的
                        noise[j] = np.random.uniform((type - temp_b) - type * k, type - temp_b)
                        a_new[j] = temp_b + noise[j]
                else:  # 原始出价合理[0, type]
                    if random.random() < 0.9:
                        noise[j] = np.random.uniform(0 + (type - temp_b) * (1 - k), type - temp_b)  # 向type靠近
                        a_new[j] = temp_b + noise[j]
                    else:
                        noise[j] = np.random.uniform(- temp_b * k, 0)
                        a_new[j] = noise[j] + temp_b

        # 加了noise的actions  discrete+a_new
        final_a = a_discrete + a_new

        return a_discrete, a_new, a_continuous_original, noise

    def chooseAction(self, state, trader_type, type, count, market_number, a, b):
        #action 产生离散 and 连续的动作，选择一个
        continuous_action = [0] * self.action_dim[1]
        choose_discrete = 0
        choose_continuous_param = 0
        state = np.reshape(state, (1, 1, self.state_dim))
        # 返回离散动作， 连续动作，原始连续参数，加的noise
        discrete_action, continuous_action, orignal_continuous_action, noises = self.actor_output(state, trader_type, type, count, market_number, a, b)
        # 根据此，选择合适的动作对
        if random.random() > self.epsilon:
            choose_discrete = np.argmax(discrete_action)  # 选择值最大的discrete action
            choose_continuous_param = continuous_action[int(choose_discrete)]
            #discrete_action = np.argmax(self.get_qs(state, continuous_action))  # 选择Q值最大的index
        else:
            choose_discrete = np.random.randint(0, self.action_dim[0])
            choose_continuous_param = continuous_action[int(choose_discrete)]
        # action = {k, xk} xk 是所有的参数
        # zip成一维数组
        dis_continuous_action = discrete_action + continuous_action

        return [choose_discrete, choose_continuous_param], dis_continuous_action, orignal_continuous_action, noises  # 二维
    ...
    def choose_continuous_action(self, state, trader_type, type, count : int, market_number):  # count 代表第多少个周期
        discrete_a, continuouts_a = self.act_best_response(np.reshape(state, (1, 1, self.state_dim)))  # type (1,1,2)  (1,1,2)
        a_original_continuous = copy.copy(continuouts_a)  # 对连续action加噪音
        noise = [0] * self.action_dim[1]
        a_new = [0] * self.action_dim[1]
        #a = np.random.normal(a, self.var)  # normal加噪音
        #noise = a - a_original
        ...
        return np.array(discrete_a[0][0]).tolist(), a_new, np.array(continuouts_a[0][0]).tolist(), noise
